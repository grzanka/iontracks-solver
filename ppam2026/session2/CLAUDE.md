...
`↑ Session 2` linking to `README.md`, plus `Prev`/`Next` linking to the
...
Everything in this session happens inside opencode — no separate
"Terminal" steps in task lists (see [`session1/06`](../session1/06-exercise-diagnosis.md)
for why). Prompts for the agent go in fenced code blocks (plain, no
`bash` tag, not quoted strings) — copy-pasteable, consistent with
`session1/06`'s "Example prompt" blocks.
